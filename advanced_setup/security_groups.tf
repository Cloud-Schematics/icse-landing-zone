...
locals {
  security_group_json = jsondecode(file("${path.module}/json-config/template-security-groups.json"))
...
module "security_groups" {
  ...
  resource_group_id = each.value.resource_group_id == null ? var.resource_group_vpc_map[each.value.vpc_name] : each.value.resource_group_id
  security_groups = [
    {
